...
public class PlayerControllerAndroid : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (!dead)
        {
            xAxis = CrossPlatformInputManager.GetAxis(GlobalVariables.HORIZONTAL);

            isJumping = CrossPlatformInputManager.GetButtonDown(GlobalVariables.JUMP);

            if (CrossPlatformInputManager.GetButton(GlobalVariables.ATTACK))
                rbAnimator.SetBool("isAttacking", true);
            else
                rbAnimator.SetBool("isAttacking", false);

            if (CrossPlatformInputManager.GetButton(GlobalVariables.BLOCK))
            {
            ...
            {
                rb2d.velocity = new Vector2(xAxis * 1.2f, rb2d.velocity.y);
                rbAnimator.SetBool("isRunning", false);
                rbAnimator.SetBool("isJumping", true);
            }
        }
        ...
        ///
        ///Works on mobile devices when pressing Back button
        ///
        if (Input.GetKeyDown(KeyCode.Escape))
        {
            ...
            {
                if (pauseObject.activeSelf == false)
